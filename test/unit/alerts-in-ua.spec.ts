...
describe('AlertsInUaService', () => {
  ...
  describe('notifyByUser method', () => {
    it('should construct a notification body for the single alert', async () => {
      ...
      await alertsInUaService.notifyByUser(testUser, true);
    ...
    it('should handle the absence of alerts while requesting general information', async () => {
      receiveMethodSpy.mockResolvedValueOnce([]);
      ...
      expect(notifySpy).toHaveBeenCalledWith(
        testUser.phone_number,
        'There are no alerts for the civilians in Ukraine',
      );
    });
